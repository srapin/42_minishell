...
/*   Updated: 2023/06/21 01:57:10 by hlesny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    parse_current_filename(t_filename **filenames, t_filename *current_f, t_token_list *current, size_t prefix_len)
{
    int     i;
    ...
    while (i < current->length)
    {
        ...
        {
            filename_pos = ft_search_str_in_str(filename_pos, ft_substr(current->content, i, next_wcard_index - i));
            if (!filename_pos)
            {
            ...
            while (current->content[i] && current->content[i] == '*' && !is_in_quotes(current, i))
                i++;
        }
        else
            break ;
