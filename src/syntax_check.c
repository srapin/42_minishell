...
/*   Updated: 2023/06/20 01:44:12 brapi agiguair         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    check_io_redirect(t_data *data, t_token_list **op)
{
    ...
    if ((*op)->length > 2)
        display_se(data, ft_substr((*op)->content, 2, 2)); // garbage collector ? permettrait d'y ajouter le malloc de ft_substr et reglerait le probleme de ou peut le free
    current = (*op)->next;
    while (current && current->type == whitespace)
        current = current->next;
...
void    check_control_op(t_data *data, t_token_list **current) // ie "||" or "&&"
{
    t_token_list    *curr;
    ...
    if (!curr) // faut-il redonner le prompt ?
        display_se(data, ft_strdup("newline")); // ou ft_substr((*current)->content, 0, 2) ? 
